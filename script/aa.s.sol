...
contract AAAA is Test, Script, Constants, Config {
    ...
    function run() external {
        for (uint i=31; i <40; i++) {
            ...
            emit log_bytes(abi.encode(pool));

            bytes memory hookData = new bytes(0);

            vm.broadcast();
            IPoolManager(POOLMANAGER).initialize(pool, startingPrice, hookData);
            ...
            IPositionManager(address(posm)).mint(
                pool, tickLower, tickUpper, liquidity, amount0Max, amount1Max, msg.sender, block.timestamp + 100 * i, hookData
            );
            vm.stopBroadcast();
        }
    }
